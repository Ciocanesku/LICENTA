...
const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
        user: 'shop.ckn.off@gmail.com',  // ✉️ Adresa ta de Gmail
        pass: 'oyuu gsxj ecyb qblv'   // 🔑 Parola de aplicație generată
    }
});
...
const sendOrderConfirmationEmail = async (email, orderId, cart, totalPrice, discount, finalTotal, address, paymentMethod) => {
    let orderDetails = cart.map(item => 
        `${item.quantity}x ${item.name} - $${item.price} (Total: $${(item.price * item.quantity).toFixed(2)})`
    ).join('<br>');

    let discountText = discount > 0 
        ? `<p><strong>Reducere aplicată:</strong> -${discount}%</p>
           <p><strong>Total cu reducere:</strong> $${finalTotal.toFixed(2)}</p>` 
        : `<p><strong>Total:</strong> $${finalTotal.toFixed(2)}</p>`;

    const mailOptions = {
        from: '"CKN Shop" <emailultau@gmail.com>',
        to: email,
        ...
        html: `<h3>Mulțumim pentru comandă!</h3>
               ...
               <p>${orderDetails}</p>
               <p><strong>Total înainte de reducere:</strong> $${totalPrice.toFixed(2)}</p>
               ${discountText}
               <hr>
